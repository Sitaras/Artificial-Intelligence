...
class CSP(search.Problem):
    ...
    def __init__(self, variables, domains, neighbors, constraints, c):
        ...
        self.sumOfCheck=0
        self.weight={} #
        # a dictionary which is used in dom/wdeg heuristic (domdivwdeg)
        # All constraints of the problem start with weight w = 1.
        # Whenever a propagator leads to a failure, the weight of the corresponding constraint is increased by 1.
        for key in c.keys():
            self.weight[(key[0],key[1])]=1

        self.last_variable=None # this is used in cbj_search to save the dead-end variable
        self.prev_conflict=[]   # this list is used in cbj_search to save the set of coflict of the dead-end variable
        self.conflict_set={}    # a dictionary which is used in cbj_search to save the set of conflict of every variable
...
def revise(csp, Xi, Xj, removals, checks=0):
    ...
    for x in csp.curr_domains[Xi][:]:
        ...
        if conflict:
            ...
            revised = True
    if not csp.curr_domains[Xi]:
        # failure, the weight of (Xi,Xj) constraint is increased by 1
        csp.weight[(Xi,Xj)] += 1
...
def domdivwdeg(assignment, csp):
    # the dom/wdeg heuristics selects the variable with the lowest ratio dom / wdeg.
    # every variable is assigned a weighted degree, which is the sum of the weights of all the constraints it participates.
    if csp.curr_domains is None:
        return first_unassigned_variable(assignment, csp)

    min_var = float("Inf")
    min_value = float("Inf")
    sum = {} # here save the weighted degree for every variable
    for i in csp.variables:
        if i in assignment:
            continue
        sum[i] = 0
        # calculate the weighted degree of i variable
        for j in csp.neighbors[i]:
            if j not in assignment:
                sum[i] += csp.weight[(i, j)]

        if sum[i]==0:
            continue
        # for the variable i, take the number of available values (from the domain)
        l = len(csp.curr_domains[i])
        # find the variable with smallest ratio dom / wdeg
        if  min_value > l /sum[i]:
            min_value = l /sum[i]
            min_var=i
    ...
    for B in csp.neighbors[var]:
        if B not in assignment:
            ...
            if not csp.curr_domains[B]:
                # failure, the weight of (B,var) constraint is increased by 1
                csp.weight[(B,var)] += 1
                return False,check
    return True,check
...
#------------------ Backtracking ------------------
def backtracking_search(csp, select_unassigned_variable=domdivwdeg,
...
def cbj_search(csp,select_unassigned_variable = domdivwdeg,
               order_domain_values = lcv,inference = forward_checking):

    def update_conflicts(var,csp):
        # update the set of conflicts of every variable.
        # add the new variable, which assigned, to which set it is inconsistent.
        for v in csp.variables:
            if var in csp.neighbors[v] and var not in csp.conflict_set[v]:
                csp.conflict_set[v].append(var)
        return csp.conflict_set

    def merging(var,assigned_variables,csp):
        for v in csp.variables:
            temp = []
            if v == var:
                csp.conflict_set[v] += csp.prev_conflict # merging
    ...
    def cbj(assignment):
        ...
        for value in order_domain_values(var, assignment, csp):
            if csp.nconflicts(var, value, assignment) == 0:
                ...
                csp.conflict_set = update_conflicts(var, csp)
                removals = csp.suppose(var, value)
                condition,check=inference(csp, var, value, assignment, removals)
                csp.sumOfCheck+=check
                if condition:
            ...
            # check if last (previous) variable (last_variable) had a dead end
            # and if the current variable belongs to its set of conflicts.
            # (try to find the variable at the set of conflicts of variable that dead-end happened which belongs deepest in the search tree,
            # when found this variable do merging of the conflicts sets and then retreat at this.)
            if csp.last_variable != None and var in csp.conflict_set[csp.last_variable]:
                # Yes,found it,then merge the sets of conflicts (add the conflict set of dead-end variable to the conflict set of current variable)
                csp.last_variable = None
                csp.conflict_set = merging(var,assigned_variables,csp)
            csp.unassign(var, assignment)
